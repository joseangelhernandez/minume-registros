...
function CalifTable(calificaciones) {
  ...
  function EnviarCalificaciones(){
    try{
      axios.put('http://jose03-001-site1.htempurl.com/api/USUARIOROLE_SP/'+`${cookies.get('usuario')}?sesion_trabajo=3`)
      .then(()=> {
  ...
  function ActivarEnvios(){
    ...
    calificaciones.Lista_usuarios.forEach(element => {
      try{
        axios.put('http://jose03-001-site1.htempurl.com/api/USUARIOROLE_SP/'+`${element.usuario}?sesion_trabajo=2`);
      }catch(error){
        console.log(error);
      }
      itemsProcesados++;
