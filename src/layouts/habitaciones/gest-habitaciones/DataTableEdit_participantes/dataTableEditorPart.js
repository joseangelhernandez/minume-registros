...
function CalifTable(tblParticipantes) {
  const {auth} = useAuth();
  const cookies = new Cookies();
  const history = useNavigate();
  const url = 'https://minumeapi.azurewebsites.net/api/HABITACIONES/PARTICIPANTES';
  let dataUpdate = [];
  let index = 0;
  ...
  const [data, setData] = useState(tblParticipantes.tblParticipantes);
  const [habitaciones, setHabitaciones] = useState();

  useEffect(()=>{
    jwtInterceoptor.get('https://minumeapi.azurewebsites.net/api/HABITACIONES/PARTICIPANTES')
    .then((response)=> {
  ...
  const columns = [
    ...
    { title: "Nombre del participante", field: "nombre", editable: 'never',
      headerStyle: {
        fontSize: '0.8rem',
        fontWeight: '700',
        opacity: '0.7',
  ...
  return (
    <MaterialTable
    title="Asignación de habitaciones a participantes" 
    data={data} 
    columns={columns}
    icons={tableIcons}
    actions={[
