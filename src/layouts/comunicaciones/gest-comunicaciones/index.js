...
function ProductsList({ socket, usuarios }) {
  ...
  const handleNotificacion = (type, usuarioRecibe, nombreUsuario) => {
    axiosOrigin.get('https://localhost:44315/api/NOTIFICACION/22')
    .then((response)=> {console.log(response.data)})

    console.log(auth.usuario, usuarioRecibe, nombreUsuario, Number(type));
    axiosOrigin.post('https://localhost:44315/api/NOTIFICACION'+`/${usuarioRecibe}`,
    {
